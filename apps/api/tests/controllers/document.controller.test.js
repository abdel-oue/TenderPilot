...
describe('DocumentController.file (EX-03)', () => {
  it('serves the PDF inline, so the browser viewer honours #page=N', async () => {
    ...
    await new DocumentController(service).file({ params: { id: UUID } }, reply);
  ...
  it('rejects a malformed id before touching the filesystem', async () => {
    ...
    await expect(
      new DocumentController(service).file({ params: { id: '../../etc/passwd' } }, fakeReply()),
...
describe('DocumentService.getPages (EX-07)', () => {
  ...
  it('keeps an unreadable page in the list and marks it as such', async () => {
    const result = await new DocumentService(documents).getPages(UUID);
  ...
  it('throws NOT_FOUND for an unknown document', async () => {
    const empty = { async findById() { return undefined; }, async findChunks() { return []; } };
    await expect(new DocumentService(empty).getPages(UUID)).rejects.toMatchObject({
