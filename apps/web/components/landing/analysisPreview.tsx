  ...
  return (
    <div className="card analysis-preview" data-testid="analysis-preview">
      ...
      <div className="analysis-content" role="tabpanel" id={`analysis-panel-${tab}`} aria-labelledby={`analysis-tab-${tab}`} tabIndex={0} data-testid="analysis-panel">
        ...
        {tab === 0 && <><div className="verdict-note"><CircleAlert size={19} /><div><strong>{demo.verdict}</strong><p>{demo.verdictText}</p></div></div><p className="analysis-subtitle">{demo.next}</p><p className="analysis-next">{demo.nextText}</p><button className="text-link" onClick={() => { setTab(2); setSourceOpen(true); }} data-testid="view-source">{demo.openSource}<ArrowUpRight size={16} /></button></>}
