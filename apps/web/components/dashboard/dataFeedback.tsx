...
export function DataFeedback({ pending, error, retry }: DataFeedbackProps) {
  if (pending) return <div className="space-y-5" aria-label="Chargement des dossiers" data-testid="data-loading"><div className="grid grid-cols-2 gap-4 md:grid-cols-4">{[0, 1, 2, 3].map((key) => <div key={key} className="h-36 animate-pulse rounded-2xl bg-soft" />)}</div><div className="h-80 animate-pulse rounded-2xl bg-soft" /></div>;
