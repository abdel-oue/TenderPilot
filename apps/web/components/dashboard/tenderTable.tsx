...
export function TenderTable({ tenders, compact = false }: TenderTableProps) {
  ...
  return (
    <section className={cn(CARD, "overflow-hidden")} data-testid="tender-list">
      ...
      {rows.length ? <div className="divide-y divide-border">{rows.map((tender) => <Link key={tender.id} href={`/tenders/${tender.id}`} className="group flex items-center gap-3 px-4 py-3 transition duration-200 hover:bg-soft/60 md:gap-4" data-testid="tender-row"><span className="hidden h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-border text-accent md:flex"><FolderOpen size={16} strokeWidth={1.5} /></span><div className="min-w-0 flex-1"><p className="text-mini text-muted">{tender.reference}</p><p className="truncate text-xs font-medium">{tender.title ?? "Dossier sans titre"}</p><p className="truncate text-mini text-muted">{tender.buyer ?? "Acheteur non renseigné"}</p></div><div className="hidden text-right md:block"><p className="text-mini text-muted">Date limite</p><p className="text-xs">{shortDate(tender.deadline)}</p></div><TenderStatus tender={tender} /><ArrowUpRight size={16} className="hidden text-muted transition group-hover:text-accent md:block" /></Link>)}</div> : <div className="flex flex-col items-center px-6 py-10 text-center" data-testid="tenders-empty"><FolderOpen size={26} strokeWidth={1.3} className="mb-3 text-accent" /><h3 className="font-heading text-lg">{tenders.length ? "Aucun dossier correspondant" : "Votre prochaine opportunité vous attend."}</h3><p className="mt-1.5 max-w-sm text-xs leading-5 text-muted">{tenders.length ? "Essayez une autre recherche ou un autre filtre." : "Importez votre premier appel d’offres pour commencer à y voir plus clair."}</p>{tenders.length ? <button className="mt-5 cursor-pointer text-sm font-medium text-accent" onClick={() => { setSearch(""); setFilter("all"); }}>Réinitialiser les filtres</button> : <Link href="/tenders/new" className={cn(PRIMARY, "mt-6")}>Créer mon premier dossier</Link>}</div>}
    </section>
  );
}
