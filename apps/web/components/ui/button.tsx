...
export function Button({ children, variant = "secondary", href, onClick, className, disabled, ...rest }: ButtonProps) {
  const classes = cn(BASE, "disabled:cursor-not-allowed disabled:opacity-50", variant === "primary" && PRIMARY, className);
  if (href) {
    return (
  ...
  return (
    <button className={classes} onClick={onClick} disabled={disabled} data-testid={rest["data-testid"]}>
      {children}
    </button>
  );
}
